...
function validateContactName(){
	...
	if ($_SESSION['ct_type'] == "Business"){
		if (!isset($_POST['ct_disp_name'])){
			$err_msgs[] = "A business name must be specified";
		} else {
			$dname = trim($_POST['ct_disp_name']);
			if (strlen($dname) == 0){
				$err_msgs[] = "A business name must be specified";
			} else if (strlen($dname) > 200) {
				$err_msgs[] = "The business name must be no longer than 200 characters in length.";
			}
		}
		if (count($err_msgs) == 0){
		...
		if (!isset($_POST['ct_first_name'])){
			$err_msgs[] = "A first name must be specified";
		}else {
			$fname = trim($_POST['ct_first_name']);
			if (strlen($fname) == 0){
				$err_msgs[] = "A first name must be specified";
			} else if (strlen($fname) > 100) {
				$err_msgs[] = "The first name must be no longer than 100 characters in length.";
			}
		}
		if (!isset($_POST['ct_last_name'])){
			$err_msgs[] = "A last name must be specified";
		}else {
			$lname = trim($_POST['ct_last_name']);
			if (strlen($lname) == 0){
				$err_msgs[] = "A last name must be specified";
			} else if (strlen($lname) > 100) {
				$err_msgs[] = "The last name must be no longer than 100 characters in length.";
			}
		}
		if (isset($_POST['ct_disp_name'])){
			$dname = trim($_POST['ct_disp_name']);
			if (strlen($dname) > 200) {
				$err_msgs[] = "The display name must be no longer than 200 characters in length.";
			}
		}
		if (count($err_msgs) == 0){
