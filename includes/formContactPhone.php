...
function validateContactPhone(){
	...
	if(!isset($_POST['ph_number'])){
		...
		//--------------------------------------------------------------------
		// 3. Ensures that phone number entered matches North American form formats
		// {ex.(519)555-1212, 519-555-1212}
		//--------------------------------------------------------------------

    	$validation=validatePhone($number);
		if ($validation == false) {
			$err_msgs[] = "The phone number is Invalid";}
		}
		//--------------------------------------------------------------------
...
function validatePhone($phone) {
	...
	}else if(preg_match("/^[(][0-9]{3}[)][0-9]{3}-[0-9]{4}$/i", $phone)){
		return true;
		
	}else if(preg_match("/^[+][0-9][(][0-9]{3}[)][0-9]{3}-[0-9]{4}$/i", $phone)){
		return true;
		
	}
	else if(preg_match("/^[0-9]{10}$/i", $phone)){
		return true;
	}
	else {
		return false;
